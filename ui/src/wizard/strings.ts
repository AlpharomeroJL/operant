...
// downloadErrorStrings are transcribed by hand from
// sidecars/downloader/wizard_copy.json (U2A's copy, already checked by that
// lane's check_wizard_copy.mjs for glossary terms and em dashes). They are
// kept byte-for-byte identical to that source on purpose: wizard_copy.json
// is the copy of record, this file is what actually ships. If the two ever
// drift, wizard_copy.json wins.
//
...
// New: docs/specs/zero-code.md screen 4 (guided first task: a suggested
// goal against the fixture web app, narrated explore, ending on one button
// labeled Save as workflow). Teaching is the approved user-facing word for
// what contracts/microcopy_glossary.json calls explore internally.
...
// Shell-level strings for the wizard overlay itself (not part of any one
// screen's copy in wizard_copy.json, since that file only covers the five
// screens' own content).
